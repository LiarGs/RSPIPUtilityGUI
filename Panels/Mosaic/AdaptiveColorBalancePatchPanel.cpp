#include "AdaptiveColorBalancePatchPanel.h"

namespace Panels::Mosaic {

AdaptiveColorBalancePatchPanel::AdaptiveColorBalancePatchPanel(QWidget *parent) : MosaicPanelBase(parent) {
    _SetupUi();
}

void AdaptiveColorBalancePatchPanel::_SetupUi() {
...
bool AdaptiveColorBalancePatchPanel::ValidateInput() {
    if (!MosaicPanelBase::ValidateInput())
        return false;

    if (_MaskSelector->Files().count() != _ImageSelector->Files().count()) {
        QMessageBox::warning(this, "数量不匹配",
                             "AdaptiveColorBalancePatch 算法要求掩膜文件数量必须与影像文件数量完全一致。");
        return false;
    }
    return true;
}

std::function<bool()> AdaptiveColorBalancePatchPanel::BuildTask(const QString &globalSavePath) {
    ...
    return [this, imageFiles, maskFiles, globalSavePath]() {
        PostLog(">> [AdaptiveColorBalancePatch] 开始执行...");
        ...
        try {
            ...
            PostLog(">> 正在执行自适应匀色补丁镶嵌 (耗时操作)...");
            algorithm.Execute();

            return _SaveResult(algorithm.AlgorithmResult, globalSavePath, "Mosaic_AdaptiveColorBalance");
